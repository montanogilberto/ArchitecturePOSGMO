...
def get_generation_rules() -> dict:
    ...
    return {
        ...
        "database": [
            ...
            "Column names: snake_case (first_name, last_name, created_At, updated_at).",
        ...
        "backend": [
            "Two files per module: modules/{module}.py (SP logic) and routes_/{module}.py (HTTP router).",
            "modules/{module}.py: imports `connection` from `databases`, conn = connection() at module level.",
            "modules/{module}.py: three functions — {module}_sp(json_file), all_{plural}_sp(), one_{module}_sp(json_file).",
            ...
            "routes_/{module}.py: `from modules.{plural} import ...`, router = APIRouter() with NO prefix and NO tags.",
